...
def read_along_track(config):
    ...
    ncfile = config['inputs']['input_file_reference']
    if "*" in ncfile or isinstance(ncfile, list):
        ds = xr.open_mfdataset(ncfile, decode_times=False, combine='nested', concat_dim='time')
    else:
        ds = xr.open_dataset(ncfile, decode_times=False)
...
def read_cmems_format(fcid):
    ...
    #sla_map = np.array(fcid.variables['Ha'][0, :, :]) - np.array(fcid.variables['Ht'][0, :, :]) # in meters
    # sla_map = np.array(fcid.variables['Hss'][0, :, :]) + np.array(fcid.variables['Hls1'][0, :, :]) + np.array(fcid.variables['Hb'][0, :, :])# - np.array(fcid.variables['MDH'][:, :])
    #sla_map = np.ma.masked_invalid(sla_map)
    #sla_map = np.array(fcid.variables['sla'][0, :, :])
    #sla_map = np.ma.masked_invalid(sla_map)
    lat_map = np.array(fcid.variables['latitude'][:])
    lon_map = np.array(fcid.variables['longitude'][:])
    sla_map = np.array(fcid.variables['Hss'][0, :, :]) + np.array(fcid.variables['Hls'][0, :, :]) + np.array(fcid.variables['Hb'][0, :, :])
    
    
    #lat_map = np.array(fcid.variables['lat'][:, 0])
    #lon_map = np.array(fcid.variables['lon'][0, :])
    #sla_map = np.array(fcid.variables['Ha'][0, :, :]) -  np.array(fcid.variables['Ht'][0, :, :])
    #sla_map = np.ma.masked_invalid(sla_map)
    # lat_map = np.array(fcid.variables['lat'][:])
    # lon_map = np.array(fcid.variables['lon'][:])
    #lat_map = np.array(fcid.variables['lat'][:, 0])
    #lon_map = np.array(fcid.variables['lon'][0, :])
