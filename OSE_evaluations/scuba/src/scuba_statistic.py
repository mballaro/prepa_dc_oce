...
msk1 = np.ma.masked_invalid(ssh_map_interpolated).mask
msk2 = np.ma.masked_where(np.abs(ssh_map_interpolated) > 10., ssh_map_interpolated).mask
msk3 = np.ma.masked_invalid(ssh_alongtrack).mask
msk4 = np.ma.masked_where(np.abs(ssh_alongtrack) > 10., ssh_alongtrack).mask
...
ssh_map_interpolated = np.ma.masked_where(msk, ssh_map_interpolated).compressed()

logging.info("end map interpolation")

# Remove bad values that appear on MSLA after interpolation
# ssh_alongtrack = np.ma.masked_where(np.abs(ssh_map_interpolated) > 3., ssh_alongtrack)
# lon_alongtrack = np.ma.masked_where(np.abs(ssh_map_interpolated) > 3., lon_alongtrack)
# lat_alongtrack = np.ma.masked_where(np.abs(ssh_map_interpolated) > 3., lat_alongtrack)
# time_alongtrack = np.ma.masked_where(np.abs(ssh_map_interpolated) > 3., time_alongtrack)
# ssh_map_interpolated = np.ma.masked_where(np.abs(ssh_map_interpolated) > 3., ssh_map_interpolated)

# lon_alongtrack = np.ma.masked_where(np.abs(ssh_alongtrack) > 3., lon_alongtrack)#
# lat_alongtrack = np.ma.masked_where(np.abs(ssh_alongtrack) > 3., lat_alongtrack)
# time_alongtrack = np.ma.masked_where(np.abs(ssh_alongtrack) > 3., time_alongtrack)
# ssh_map_interpolated = np.ma.masked_where(np.abs(ssh_alongtrack) > 3., ssh_map_interpolated)
# ssh_alongtrack = np.ma.masked_where(np.abs(ssh_alongtrack) > 3., ssh_alongtrack)

# mask_ssh_map_interpolated = np.ma.getmask(np.ma.masked_invalid(ssh_map_interpolated))
# mask_ssh_alongtrack = np.ma.getmask(np.ma.masked_invalid(ssh_alongtrack))

# lon_alongtrack = np.ma.masked_array(lon_alongtrack, mask=mask_ssh_alongtrack)
# lat_alongtrack = np.ma.masked_array(lat_alongtrack, mask=mask_ssh_alongtrack)
# time_alongtrack = np.ma.masked_array(time_alongtrack, mask=mask_ssh_alongtrack)
# ssh_map_interpolated = np.ma.masked_array(ssh_map_interpolated, mask=mask_ssh_alongtrack)
# ssh_alongtrack = np.ma.masked_array(ssh_alongtrack, mask=mask_ssh_alongtrack)

# lon_alongtrack = np.ma.compressed(np.ma.masked_array(lon_alongtrack, mask=mask_ssh_map_interpolated))
# lat_alongtrack = np.ma.compressed(np.ma.masked_array(lat_alongtrack, mask=mask_ssh_map_interpolated))
# time_alongtrack = np.ma.compressed(np.ma.masked_array(time_alongtrack, mask=mask_ssh_map_interpolated))
# ssh_map_interpolated = np.ma.compressed(np.ma.masked_array(ssh_map_interpolated, mask=mask_ssh_map_interpolated))
# ssh_alongtrack = np.ma.compressed(np.ma.masked_array(ssh_alongtrack, mask=mask_ssh_map_interpolated))
...
if  YAML['properties']['filtering']['filter_type'].lower() != 'none':
